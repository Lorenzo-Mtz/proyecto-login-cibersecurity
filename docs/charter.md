...
| M5 | Fase 4 completa: remediación acotada a los siete gaps de Nivel 1 identificados en la Fase 3 |
| M6 | Cierre del proyecto: Lessons Learned Log completo, glosario finalizado |
...
| 3 | Septiembre 2026 | Se agrega la tarea **4.6.4 Suite de pruebas automatizadas** al WBS y se ejecuta de forma anticipada, durante el paquete 4.1, en lugar de al cierre de la fase. Incorpora `pytest` como dependencia de desarrollo y la carpeta `tests/`, con la verificación de los paquetes 4.1, 4.4, 4.5.2 y 4.5.3 (42 pruebas). | Hasta ahora cada paquete se verificaba con scripts temporales fuera del proyecto, que se borraban: el proyecto podía demostrar que un control funcionó un día, pero no que sigue funcionando. Varias de las propiedades verificadas son invisibles al probar en el navegador (la igualdad de latencia entre un login bloqueado y uno fallido, que un intento bloqueado no se registre, que una cookie copiada antes del logout deje de servir). Se adelanta porque los paquetes 4.2 y 4.3 vuelven sobre este mismo código —4.2.6 llama a `reset_attempts` y 4.3.5 reutiliza el contador de intentos—, de modo que decidirlo en 4.6 habría llegado después de los paquetes que más lo necesitan. No cambia los criterios de éxito (Sección 3) ni las exclusiones (Sección 4) |
| 4 | Septiembre 2026 | Se fija la **edición del OWASP Top 10** usada como referencia: **Top 10:2025**. Los artefactos (Charter Sección 3, Scope Statement, WBS) decían "OWASP Top 10" sin edición, mientras que las notas de estudio de Fase 0 se escribieron contra la de 2021. El recorrido documentado del paquete 4.5.6 (`docs/fase2-owasp-top10.md`) se hizo contra la edición 2025. | Un artefacto que nombra un estándar sin fijar su versión envejece sin avisar, que es exactamente lo que R8 pide evitar. Se elige la vigente porque es lo que haría una revisión real y porque dos de sus categorías —A03 *Software Supply Chain Failures* y A10 *Mishandling of Exceptional Conditions*— cubren trabajo ya realizado (paquete 4.5.5 y LL15) que la lista de 2021 no reconocía como categoría propia. No cambia los criterios de éxito (Sección 3) ni las exclusiones del Scope (Sección 4): el marco de referencia para la autoevaluación sigue siendo ASVS Level 1 |
| 5 | Septiembre 2026 | **WBS v1.4:** la Fase 3 se desglosa a nivel de paquete de trabajo (5.1 – 5.5). Se fija **OWASP ASVS 5.0.0** como versión de referencia, se **invierte el orden** de los dos entregables —el threat model (5.2) va antes que la autoevaluación (5.3)— y se explicita que la fase **documenta, no corrige**. La deuda abierta de la Fase 0 (tema 0.1 HTTP/HTTPS y la entrada 2.7.3 del Lessons Learned) se asigna formalmente a los paquetes 6.1 y 6.2 del cierre del proyecto. | Rolling wave planning: la Fase 3 se detalla al cerrar la Fase 2 y antes de ejecutarla, que es la mitigación acordada para R2. La versión se fija por la misma razón del cambio #4: un artefacto que nombra un estándar sin versión envejece sin avisar (R8). El orden se invierte porque modelar amenazas primero hace que los resultados del checklist se lean por riesgo real y no por orden de capítulo. Que la fase solo documente es R1 aplicado: el criterio de aceptación (Sección 3) pide estatus *pass/fail*, así que un `fail` documentado cumple, e implementar las correcciones sería alcance nuevo — se propone como fase aparte en 5.4.3. No cambia los criterios de éxito (Sección 3) ni las exclusiones del Scope (Sección 4) |
| 6 | Septiembre 2026 | **Se agrega la Fase 4 — Remediación acotada**, con alcance cerrado a los **siete gaps de Nivel 1** de `docs/fase3-consolidacion.md` (G1 a G7). **WBS v1.5:** la fase se desglosa en 6.1 – 6.8 y el cierre del proyecto pasa de la sección 6.0 a la **7.0**; en consecuencia, los paquetes que el cambio #5 asignó como "6.1 y 6.2" son ahora **7.1 y 7.2** — la entrada #5 no se edita, porque reescribir el registro destruiría la trazabilidad que justifica tenerlo. Hitos: se agrega **M5 Fase 4 completa** y el cierre del proyecto pasa de M5 a **M6**. | La Fase 3 entregó 23 gaps priorizados y la decisión de qué hacer con ellos es de alcance, no técnica, así que se toma aquí. Se elige el Nivel 1 y no todo: son de costo bajo, caben en la disponibilidad de 4–8 h/semana y cada uno tiene un criterio de terminado objetivo — el requisito ASVS o la amenaza que cierra. Dos de ellos, **G2 y G3, son de una línea cada uno** y cierran un incumplimiento de nivel 1 y una vía abierta de R9 que el propio proyecto encontró; dejarlos sin hacer era difícil de justificar. Se descarta incluir el Nivel 2 —del tamaño de la Fase 2 entera— porque con la disponibilidad actual el riesgo no es fallar sino **quedarse a medias**, que es R16. El Nivel 3 queda fuera mientras no haya despliegue (Scope, Sección 4). No cambia los criterios de éxito (Sección 3) ni las exclusiones |
